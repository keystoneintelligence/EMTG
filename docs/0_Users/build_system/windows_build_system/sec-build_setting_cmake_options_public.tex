...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\item If building with SNOPT, ensure ENABLE\_SNOPT is checked and SNOPT\_MINGW\_DLL is checked
				\begin{figure}[H]
					\centering
				...
				\end{figure}
		\end{enumerate}
		\item To build EMTG with \ac{IPOPT} instead of SNOPT, use the same ``Ungrouped Entries'' section and set the following:
		\begin{enumerate}
			\item Check ENABLE\_IPOPT
			\item Uncheck ENABLE\_SNOPT if SNOPT is not available
			\item If \ac{IPOPT} was built with MinGW and EMTG is being built with \ac{MSVC}, ensure IPOPT\_MINGW\_DLL is checked
			\item Confirm IPOPT\_ROOT\_DIR, IPOPT\_INCLUDE\_DIR, IPOPT\_LIBRARY\_DIR, IPOPT\_BIN\_DIR, and optionally IPOPT\_MINGW\_BIN\_DIR point to the \ac{IPOPT} installation
		\end{enumerate}
