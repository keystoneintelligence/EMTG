...
\indent\listitem{\ac{NLP} with initial guess}{Leverages a gradient-based solver to solve a problem given an initial guess. EMTG supports IPOPT and optional SNOPT backends. More detail is provided in \ref{sec:gradient_based_solver}. Users must provide an initial guess as a {\tt .emtg} solution file via the ``Trial decision vector or initial guess'' input option.}
...
\noindent Since most interplanetary trajectory optimization problems may consist of hundreds of variables and tens to hundreds of constraints, these problems are best solved with sparse \ac{NLP} solvers. \ac{EMTG} uses IPOPT by default and may also be built with SNOPT.
...
\begin{enumerate}
    ...
    \item \textbf{\ac{NLP} solver:} Allows users to choose a solver compiled into the current build. IPOPT is the open-source default; SNOPT is optional. Legacy WORHP value 1 is unsupported.
        \begin{table}[H]
            \hspace{2cm}
            \begin{tabular}{lp{5cm}}
