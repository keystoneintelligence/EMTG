...
try:
    from . import JourneyOptions
except ImportError:  # historical scripts import MissionOptions as a top-level module
    import JourneyOptions


def _parse_user_data(text):
    entries = []
    start = 0
    depth = 0
    quote = None
    ...
    for entry in entries:
        pair = ast.literal_eval(entry.strip())
        if not isinstance(pair, tuple) or len(pair) != 2 or not isinstance(pair[0], str):
            raise ValueError("user_data entries must be (string, literal) pairs")
            ...
            if line.strip('\r') != "":
                if line[0] != "#":
                    ...
                    if linecell[0] == "mission_name":
                        ...
                        full_notes = line[len("user_data"):].lstrip(" ").rstrip(" \r\n")
                        self.user_data = _parse_user_data(full_notes) if full_notes.strip() else {}
