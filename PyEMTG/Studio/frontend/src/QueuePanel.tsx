...
import { api } from './api'
import type { BodyOption, Job } from './types'
...
export function QueuePanel({ jobs, globalCores, refresh }: { jobs: Job[]; globalCores: number; refresh: () => void }) {
  ...
  useEffect(() => {
    api.searchDefaults().then(value => {
      const serialized = JSON.stringify(value.config, null, 2)
    ...
    }).catch(value => { setError(String(value)); setDefaultsMessage('Unable to discover EMTG runtime assets') })
  }, [])
  ...
  async function create() {
    try {
      ...
      await api.createJob({ name, requested_cores: cores, queue: true, config: JSON.parse(config) })
      refresh()
  ...
  return <div className="queue-panel">
    <section className="queue-list">
      ...
      {jobs.map(job => {
        ...
        return <article className={`job job-${job.status}`} key={job.id}>
          ...
          <small>{job.effective_cores}/{job.requested_cores} cores · {String(job.progress.checkpoint_status || 'not started')}</small>
          <div className="job-actions">
            ...
            <input title="Requested cores" type="number" min={1} value={job.requested_cores} onChange={event => api.setJobCores(job.id, Number(event.target.value)).then(refresh)} />
          </div>
    ...
    <section className="new-search">
      ...
      <div className="form-row"><label>Name<input value={name} onChange={event => setName(event.target.value)} /></label><label>Cores<input type="number" min={1} value={cores} onChange={event => setCores(Number(event.target.value))} /></label></div>
      ...
      <button className="primary" disabled={!syntheticDebug && !defaultsReady} onClick={create}>{syntheticDebug ? 'Validate and queue synthetic debug search' : 'Validate and queue real EMTG search'}</button>
    </section>
  </div>
}
